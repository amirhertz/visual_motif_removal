...
class UnetDiscriminatorD(nn.Module):
    def __init__(self, in_channels=3, depth=6, blocks=1, size=128, start_filters=8, residual=True, batch_norm=True):
        super(UnetDiscriminatorD, self).__init__()
        self.optimizer = None
        self.encoder = UnetEncoderD(in_channels=in_channels, depth=depth, blocks=blocks, start_filters=start_filters,
                                    residual=residual, batch_norm=batch_norm)
        self.conv = nn.Conv2d(start_filters * (2 ** (depth-1)),
                              start_filters * (2 ** (depth-2)), size // (2 ** (depth-1)), stride=1)
        self.fc = nn.Sequential(
            nn.Linear(start_filters * (2 ** (depth-2)), 50),
    ...
    def forward(self, image):
        ...
        output = self.conv(output).view(output.size()[0], -1)
